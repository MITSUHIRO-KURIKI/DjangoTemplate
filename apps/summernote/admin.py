...
class SummernoteAttachmentAdmin(admin.ModelAdmin):

    # ユーザアイコンの表示
    def image_file_preview(self, obj):
        try:
             return mark_safe(f'<img src="{obj.image_file.url}" style="width:25px;height:25px;">')
        except: pass
    image_file_preview.short_description = 'Image'

    # 一覧画面
    list_display_ = ('attachment_id',
                     'image_file_preview',
                     'date_create',)
    list_filter   = []
...
admin.site.register(SummernoteAttachment, SummernoteAttachmentAdmin)