...
class SampleItemsListView(ListView):
    ...
    template_name         = 'sample/items/items.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        request_sort     = 'asc' if self.request.GET.get('sort', 'asc') == 'asc' else 'desc'
        url_param = {
            'page': request_page,
        ...
        context.update({
            ...
            'search_form_action_url': reverse('sample:items'),
            'pagenate':               pagenate,
            'url_param':              url_param,
        })
        return context
...
class SampleItemsDetailView(DetailView):
    ...
    model               = SampleAnimeDataset
    template_name       = 'sample/items/detail.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        cytoscapeInputData = "[\n"
        for s in nodes_data:
            cytoscapeInputData += "{ group: 'nodes', data: {"
            cytoscapeInputData += str(s)
            cytoscapeInputData += "}},\n"
        for s in edges_data:
            cytoscapeInputData += "{ group: 'edges', data: {"
            cytoscapeInputData += str(s)
            cytoscapeInputData += "}},\n"
        cytoscapeInputData += "]"
        # 共起ネットワーク作成△
        ...
        context.update({
            'NETWORK_TYPE':            'undirected', # 'undirected' or 'directed'
            'IS_USE_EXPAND_COLLAPSE':  False,
            'IS_USE_BIRDs_NAVIGATION': True,
            'cytoscapeInputData':      cytoscapeInputData,
            'AjaxEndPointLlm':         reverse('sample:llm_summary_ajax'),
            'AjaxEndPointWordCloud':   reverse('sample:create_wordcloud_ajax'),
        })
        return context